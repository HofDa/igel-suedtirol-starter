    ...
    <>
      <section className="relative overflow-hidden bg-emerald-950 py-20 text-white md:py-28">
        ...
        <div className="container-page relative grid items-center gap-12 lg:grid-cols-[1.2fr_0.8fr]">
          <div>
            <p className="font-bold uppercase tracking-[0.2em] text-amber-300">{t('eyebrow')}</p>
            <h1 className="mt-4 max-w-4xl text-5xl font-black leading-[1.03] md:text-7xl">{t('title')}</h1>
            <p className="mt-6 max-w-2xl text-xl text-emerald-50/80">{t('intro')}</p>
            <div className="mt-9 flex flex-wrap gap-4">
              <Link href="/melden" className="inline-flex min-h-14 items-center gap-2 rounded-full bg-amber-500 px-7 font-black text-emerald-950 hover:bg-amber-400">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {features.map(({Icon, title, text}) => (
            <article key={title} className="card p-7">
              <Icon className="text-amber-700" size={36} aria-hidden="true" />
              <h2 className="mt-5 text-xl font-black text-emerald-950">{t(title)}</h2>
              <p className="mt-2 text-emerald-950/70">{t(text)}</p>
            </article>
          ))}
        </div>
      </section>
